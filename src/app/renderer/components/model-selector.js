...
class ModelSelector {
  constructor() {
    ...
    this.autoMode = false;
    this.autoSyncToProduction = false; // Sincronización automática deshabilitada por defecto
  ...
  init() {
    ...
    // Cargar modelos desde producción si existe última sincronización
    const lastSync = localStorage.getItem('lastProductionSync');
    if (lastSync) {
      // Cargar desde producción en background (sin bloquear UI)
      setTimeout(() => {
        this.loadModelsFromProduction().catch(err => {
          console.warn('No se pudo cargar desde producción al iniciar:', err);
        });
      }, 1000);
    }
  ...
  setupEventListeners() {
    ...
    const saveToProductionBtn = document.getElementById('saveToProductionBtn');
    if (saveToProductionBtn) {
      saveToProductionBtn.addEventListener('click', async () => {
        await this.saveModelsToProduction();
      });
    }

    const loadFromProductionBtn = document.getElementById('loadFromProductionBtn');
    if (loadFromProductionBtn) {
      loadFromProductionBtn.addEventListener('click', async () => {
        await this.loadModelsFromProduction();
      });
    }
  }
  ...
  toggleSelection(modelId, checkbox) {
    ...
    try {
      ...
      // Sincronizar con producción si está habilitado
      if (this.autoSyncToProduction) {
        this.syncSelectedModelsToProduction().catch(err => {
          console.warn('Error sincronizando a producción (no crítico):', err);
        });
      }
    } catch (e) {
  ...
  /**
   * Sincronizar modelos seleccionados a producción (config/models.json)
   */
  async syncSelectedModelsToProduction() {
    try {
      if (!window.qwenValencia || !window.qwenValencia.saveModelsConfig) {
        console.warn('IPC no disponible para guardar modelos a producción');
        return { success: false, error: 'IPC no disponible' };
      }

      // Leer configuración actual
      const readResult = await window.qwenValencia.readModelsConfig();
      if (!readResult.success) {
        throw new Error(readResult.error || 'Error leyendo configuración');
      }

      const currentConfig = readResult.data || {};

      // Convertir selectedModels a formato de configuración
      const updatedConfig = this.convertSelectedModelsToConfig(this.selectedModels, currentConfig);

      // Guardar en archivo
      const saveResult = await window.qwenValencia.saveModelsConfig(updatedConfig);
      if (!saveResult.success) {
        throw new Error(saveResult.error || 'Error guardando configuración');
      }

      console.log('✅ Modelos sincronizados a producción exitosamente');
      return { success: true };
    } catch (error) {
      console.error('❌ Error sincronizando modelos a producción:', error);
      return { success: false, error: error.message };
    }
  }

  /**
   * Guardar modelos a producción (con confirmación y notificación)
   */
  async saveModelsToProduction() {
    try {
      if (!window.qwenValencia || !window.qwenValencia.saveModelsConfig) {
        const error = 'No se puede guardar a producción: IPC no disponible';
        this.showNotification(error, 'error');
        return { success: false, error };
      }

      // Mostrar indicador de carga
      this.showNotification('Guardando modelos a producción...', 'info');
      ...
      localStorage.setItem('selectedModels', JSON.stringify(this.selectedModels));
      localStorage.setItem('lastProductionSync', new Date().toISOString());

      const successMsg = `✅ Modelos guardados a producción exitosamente${saveResult.backupPath ? ' (backup creado)' : ''}`;
      this.showNotification(successMsg, 'success');

      console.log('✅ Modelos guardados a producción:', {
        models: this.selectedModels,
        backupPath: saveResult.backupPath,
        timestamp: saveResult.timestamp
      });

      return { success: true, backupPath: saveResult.backupPath };
    } catch (error) {
      console.error('❌ Error guardando modelos a producción:', error);
      const errorMsg = `Error al guardar modelos: ${error.message}`;
  ...
  /**
   * Convertir selectedModels (array) a formato config/models.json
   */
  convertSelectedModelsToConfig(selectedModels, currentConfig) {
    ...
    const selectedConfig = {
      ...
      maxMode: localStorage.getItem('maxMode') === 'true',
    ...
    // Guardar selección en configuración
  ...
  /**
   * Cargar modelos desde configuración de producción
   */
  async loadModelsFromProduction() {
    try {
      if (!window.qwenValencia || !window.qwenValencia.readModelsConfig) {
        console.warn('IPC no disponible para cargar modelos desde producción');
        return { success: false, error: 'IPC no disponible' };
      }
      ...
      // Cargar modelos seleccionados desde configuración
      if (config.selected && config.selected.models) {
        const modelIds = config.selected.models.filter(m => m.enabled !== false).map(m => m.id);
        ...
        localStorage.setItem('selectedModels', JSON.stringify(this.selectedModels));
        if (config.selected.autoMode !== undefined) {
          this.autoMode = config.selected.autoMode;
        ...
        console.log('✅ Modelos cargados desde producción:', modelIds);
        return { success: true, models: modelIds };
      }

      return { success: true, models: [] };
    } catch (error) {
      console.error('❌ Error cargando modelos desde producción:', error);
      return { success: false, error: error.message };
    }
  }
  ...
  filterModels(term) {
    ...
    headers.forEach(header => {
      ...
      while (
        current &&
        !current.classList.contains('model-group-divider') &&
        !current.classList.contains('model-group-header')
      ) {
  ...
  showTooltip(event, element) {
    ...
    this.tooltipTimeout = setTimeout(() => {
      ...
      if (tooltipTokens) {
        const tokensText =
          tokens === 'N/A' || tokens === '-' || tokens === 'Ilimitado'
            ? 'Contexto: Ilimitado'
...
// ═══════════════════════════════════════════════════════════════════
// EXPOSICIÓN GLOBAL DE FUNCIONES DE PERSISTENCIA
// ═══════════════════════════════════════════════════════════════════

if (typeof window !== 'undefined') {
  // Exponer funciones de persistencia cuando modelSelector esté listo
  const exposePersistFunctions = () => {
    if (modelSelector) {
      window.saveModelsToProduction = async () => {
        return await modelSelector.saveModelsToProduction();
      };

      window.loadModelsFromProduction = async () => {
        return await modelSelector.loadModelsFromProduction();
      };

      window.syncModelsToProduction = async () => {
        return await modelSelector.syncSelectedModelsToProduction();
      };
    }
  };

  // Exponer después de inicializar
  setTimeout(exposePersistFunctions, 500);

  // También exponer si modelSelector ya existe
  if (modelSelector) {
    exposePersistFunctions();
  }
}
