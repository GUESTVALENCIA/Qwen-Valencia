...
class ErrorHandler {
  ...
  showUserFriendlyMessage(errorInfo) {
    ...
    const message = this.getUserFriendlyMessage(error, type, severity);
    const toastType =
    ...
    const messages = {
      [ErrorType.API]: {
        [ErrorSeverity.CRITICAL]:
          'Error crítico al comunicarse con el servidor. Por favor, intenta de nuevo.',
        [ErrorSeverity.HIGH]:
          'Error al procesar tu solicitud. Verifica tu conexión e intenta de nuevo.',
      ...
      [ErrorType.STORAGE]: {
        [ErrorSeverity.CRITICAL]:
          'No se puede guardar tu configuración. Verifica los permisos del navegador.',
        [ErrorSeverity.HIGH]:
          'Problema al guardar datos. Algunas configuraciones pueden no persistir.',
      ...
      [ErrorType.NETWORK]: {
        ...
        [ErrorSeverity.HIGH]:
          'Problema de conexión. Algunas funciones pueden no estar disponibles.',
        [ErrorSeverity.MEDIUM]: 'Conexión lenta o inestable.',
        [ErrorSeverity.LOW]: 'Advertencia de red menor.'
      },
      [ErrorType.PERMISSION]: {
        [ErrorSeverity.CRITICAL]:
          'Permisos necesarios denegados. Por favor, habilita los permisos requeridos.',
        [ErrorSeverity.HIGH]:
          'Algunos permisos no están disponibles. Algunas funciones pueden no funcionar.',
...
async function handleAPIError(error, source = 'api', metadata = {}) {
  ...
  if (error && typeof error.ok === 'boolean') {
    ...
    severity =
      status >= 500 ? ErrorSeverity.HIGH : status >= 400 ? ErrorSeverity.MEDIUM : ErrorSeverity.LOW;
    ...
    try {
      ...
      if (data?.error?.code) {
        ...
        errorObj = new Error(
          data?.error ?? data?.message ?? `API Error: ${status} ${error.statusText ?? 'Unknown'}`
        );
